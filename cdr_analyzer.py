...
class cdr_analyzer():
	def __init__(self,initial_json_dictionary,query_seq,end_translation_dictionaries):
		...
		try:
			self.framework1_seq = Seq(query_seq[self.framework1_frame_info['from']-1:self.framework1_frame_info['to']], IUPAC.ambiguous_dna)
		...
		try:
			self.framework2_seq = Seq(query_seq[self.framework2_frame_info['from']-1:self.framework2_frame_info['to']], IUPAC.ambiguous_dna)
		...
		try:
			self.framework3_seq = Seq(query_seq[self.framework3_frame_info['from']-1:self.framework3_frame_info['to']], IUPAC.ambiguous_dna)
		...
		try:
			self.cdr1_seq = Seq(query_seq[self.cdr1_frame_info['from']-1:self.cdr1_frame_info['to']], IUPAC.ambiguous_dna)
		...
		try:
			self.cdr2_seq = Seq(query_seq[self.cdr2_frame_info['from']-1:self.cdr2_frame_info['to']], IUPAC.ambiguous_dna)
		...
		try:
			self.v_part_of_cdr3 = Seq(query_seq[self.v_part_of_cdr3_info['from']-1:self.v_part_of_cdr3_info['to']])
		...
		try:
			...
			#where does the CDR3 loop stop in the j_gene stop translating for this particular gene
			self.j_gene_end = self.end_translation_dictionaries[self.j_gene]

			#and make it relative to the sequence read
			_end = (self.j_gene_end - int(self.j_region_info['s._start'])) + _begin 

			#j_region
			self.j_region = Seq(query_seq[_begin:_end])
			self.full_cdr3 = Seq(query_seq[self.v_part_of_cdr3_info['from']-1:_end])
		...
		self.frames_and_cdrs = {'fw1':self.framework1_seq,'cdr1':self.cdr1_seq,'cdr3':self.full_cdr3,'fw2':self.framework2_seq,'cdr2':self.cdr2_seq,'fw3':self.framework3_seq,'fw4':self.framework4_seq}
		self.frames_and_cdrs_aa = {'fw1_aa':self.framework1_seq_aa,'cdr1_aa':self.cdr1_seq_aa,'fw2_aa':self.framework2_seq_aa,'cdr2_aa':self.cdr2_seq_aa,'fw3_aa':self.framework3_seq_aa,'cdr3_aa':self.full_cdr3_aa,'fw4_aa':self.framework4_seq_aa}

		self.func_seq = self.framework1_seq + self.cdr1_seq + self.framework2_seq + self.cdr2_seq + self.framework3_seq + self.full_cdr3
		self.func_seq_aa = self.framework1_seq_aa + self.cdr1_seq_aa + self.framework2_seq_aa + self.cdr2_seq_aa + self.framework3_seq_aa + self.full_cdr3_aa
		self.unmodified['full_seq'] = str(self.func_seq).upper() + str(self.framework4_seq).upper()
		self.unmodified['full_seq_aa'] = str(self.func_seq_aa) + str(self.framework4_seq_aa)


		if str(self.func_seq_aa).find('*') != -1:
			self.unmodified['productive'] = 'False'
		else:
			self.unmodified['productive'] = 'True'

		if str(self.full_cdr3_aa).find('*') != -1:
		...
		for i,j in zip(self.frames_and_cdrs,self.frames_and_cdrs_aa):
			...
			self.frames_and_cdrs_aa[j] = str(self.frames_and_cdrs_aa[j]).upper()
